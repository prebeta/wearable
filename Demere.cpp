...
int main(int argc, char *argv[]){
	Mat src, can, blurred, background, filtered, cropped, resized;
	...
	for(i = 0; i < blurred.rows; i++){
		for(j = 0; j < blurred.cols; j++){
			...
			while(count < bkg_colors.size()){
				if(d = norm(color - bkg_colors.at(count)) < 14){
	...
	//resize(src, resized, Size(360, 480));

	//imwrite(buf, resized);		//creating templates
...
string compare_templates(Mat bkgmask){
	...
	DIR *dir;
	string dir_name = "lib\\scripts\\templates\\";
	//string dir_name = "lib/scripts/templates";
	string img_name;
	string path;
	struct dirent *ent;
	Vec3b white, color;
